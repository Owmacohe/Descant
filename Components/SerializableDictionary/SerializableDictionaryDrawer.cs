// This code copied and modified from: https://github.com/EduardMalkhasyan/Serializable-Dictionary-Unity
    ...
    public class SerializableDictionaryDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect rect, SerializedProperty prop, GUIContent label)
        {
            ...
            void Head()
            {
                ...
                void ExpandablePanel()
                {
                    ...
                    if (UnityEngine.Event.current != null && fullHeaderRect.Contains(UnityEngine.Event.current.mousePosition))
                    {
                ...
                {
                    if (UnityEngine.Event.current != null && UnityEngine.Event.current.type != EventType.Repaint)
                    {
                        return;
                    }
        ...
        void DrawListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            void Divider()
            {
                ...
                if (UnityEngine.Event.current == null || rect.Contains(UnityEngine.Event.current.mousePosition) == false)
                {
                    return;
                }

                if (UnityEngine.Event.current != null && dividerRect.Contains(UnityEngine.Event.current.mousePosition))
                {
                    if (UnityEngine.Event.current.type == EventType.MouseDown)
                    {
                        isDividerDragged = true;
                             ...
                             || UnityEngine.Event.current.type == EventType.MouseMove
                             || UnityEngine.Event.current.type == EventType.MouseLeaveWindow)
                    {
                        isDividerDragged = false;
                    }
                }

                if (isDividerDragged && UnityEngine.Event.current != null && UnityEngine.Event.current.type == EventType.MouseDrag)
                {
                    dividerPosProp.floatValue = Mathf.Clamp(dividerPosProp.floatValue + UnityEngine.Event.current.delta.x / rect.width, .2f, .8f);
                }
            }
